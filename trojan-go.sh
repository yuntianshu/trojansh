#!/bin/bash
# trojan-go一键安装脚本
# 参考: https://raw.githubusercontent.com/veip007/hj/main/trojan-go.sh
...
getMyIP(){
    # 定义外部网站列表
    websites=("-4 ip.sb" "ipv4.icanhazip.com" "-6 ip.sb" "ipv6.icanhazip.com")   

    # 循环遍历外部网站获取 IP 地址
    for website in "${websites[@]}"; do
        IP=$(curl -sL --connect-timeout 5 "$website")
        if [[ $? -eq 0 ]] && [[ -n $IP ]]; then
            break
        fi
    done
...
getData() {
    ...
    2)
        ...
        ;;
    3)
        PROXY_URL="https://killsometime.com"
        ;;
    4)
...
getCert() {
    ...
    if [[ -z ${CERT_FILE+x} ]]; then
        ...
        if [[ "$BT" = "false" ]]; then
	    # ~/.acme.sh/acme.sh   --issue -d $DOMAIN --keylength ec-256 --pre-hook "systemctl stop nginx" --post-hook "systemctl restart nginx"
            # 仅适合godaddy。需要自行在服务器设置export GD_Key="<key>" 和  export GD_Secret="<secret>"
	    ~/.acme.sh/acme.sh   --issue --dns dns_gd -d $DOMAIN --keylength ec-256 --pre-hook "systemctl stop nginx" --post-hook "systemctl restart nginx"  --standalone
        else
